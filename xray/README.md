...
重复运行脚本时，如果 OpenRC 中已有 Xray 实例正在运行，脚本会询问是否重新安装：

```text
[xray] 检测到 Xray 实例正在运行
是否重新安装 Xray (y/N) [n]:
```

默认选择 `n`，脚本会退出并保持现有实例运行。明确输入 `y` 后，脚本会先下载内核、
生成或读取新配置并完成预校验，然后停止现有实例，再替换内核和配置，以缩短服务
中断时间。

重新安装时，如果没有通过 `--config` 或 `XRAY_CONFIG` 显式指定配置文件，停止服务后
会清理当前的 `/opt/xray/config.json`，再部署脚本同目录的 `config.json` 或本次生成的
新配置。显式指定配置文件时，则直接使用指定文件进行部署。

脚本会在替换前暂存原内核和配置。如果部署后的最终配置校验失败，会恢复原内核与
配置，并重新启动原有实例。
